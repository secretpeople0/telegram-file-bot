...
async def handle_all_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. 处理管理员操作，并修复状态问题
    if "admin_act" in context.user_data and user_id == ADMIN_USER_ID:
        # 如果用户发送“取消”，则退出当前管理员操作
        if text.lower() in ["取消", "exit", "quit", "/cancel"]:
            del context.user_data["admin_act"]
            await update.message.reply_text("✅ 已取消当前管理员操作。")
            return
        # 否则执行管理员操作
    ...
    # 如果都不是，提示用户
    await update.message.reply_text("❌ 无效指令。若要取件，请发送正确的6位提取码。若要取消管理员操作，请发送“取消”。")
