...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_banned(u.id):
        ...
        return
    await update.message.reply_text("👋 发送文件即可存储，自动生成提取码")
...
async def do_gen_code(ctx):
    ...
    paths = await upload_files_to_oss(ud['file_queue'], uid, code)
    if not paths:
        await ctx.bot.send_message(uid, "❌ 上传OSS失败")
        ud['file_queue'].clear()
        return
    try:
        conn = get_db_connection()
    ...
    await ctx.bot.send_message(uid, f"🎉 提取码：`{code}`",
...
async def btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('name_'):
        ...
        await q.edit_message_text("✏️ 回复消息设置名称")
        return WAITING_FOR_NAME
    if data.startswith('skip_'):
        code = data[5:]
        await q.edit_message_text(f"✅ 已跳过 `{code}`", parse_mode='Markdown')
        return
...
def main():
    ...
    app.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND & ~filters.TEXT, handle_file))
    app.run_polling()
